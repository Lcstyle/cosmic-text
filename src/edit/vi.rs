...
fn select_in<'buffer, E: Edit<'buffer>>(editor: &mut E, start_c: char, end_c: char, include: bool) {
    ...
    let (start, end) = editor.with_buffer(|buffer| {
        ...
        'find_start: loop {
            ...
            if start.line > 0 {
                start.line -= 1;
                start.index = buffer.line_text_cow(start.line).expect("line index in bounds").len();
            } else {
                break 'find_start;
            }
        }
                        ...
                        if editor.delete_selection() {
                            ...
                            match selection {
                                ...
                                | Selection::Block(_) => {
                                    ...
                                    if after {
                                        editor.with_buffer(|buffer| {
                                            let text = buffer.line_text_cow(cursor.line).expect("cursor line in bounds");
                            ...
                            editor.with_buffer(|buffer| {
                                loop {
                                    ...
                                    if cursor.index > 0 {
                                        ...
                                        cursor.line -= 1;
                                        cursor.index = buffer.line_text_cow(cursor.line).expect("cursor line in bounds").len();
                                        continue;
                                    }
                                    break;
                                }
